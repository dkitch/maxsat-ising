...
//		}
        ...
        {
            unsigned long long first_lower_bound = (unsigned long long)(bestCost - ret);
            if(bestCost<first_lower_bound_threshold)
            {
                printf("c we arrive at upper bound %llu which is smaller than the upper_bound_threshold %llu, terminate!\n", bestCost, first_lower_bound_threshold);
                printSolution();
